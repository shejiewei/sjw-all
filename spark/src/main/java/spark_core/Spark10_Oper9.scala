...
object Spark10_Oper9 {
  def main(args: Array[String]): Unit = {
   ...
   // listRDD.collect().foreach(println)
    ...
    var distinctRDD: RDD[Int] = listRDD.distinct(3)
//    distinctRDD.collect().foreach(println//控制台打印，没有保存之前的顺序    401622337, 数据打乱重组，shuffle
    distinctRDD.collect().foreach(println)
    distinctRDD.saveAsTextFile("test/output.txt2")//数据 进行了shuffle 打乱重组，没有保存之前的顺序，每个分区存的数据和之前不一样了。
   //在spark中所有转换算子中，没有shuffle则速度快。

    val lRDD:RDD[String] = sc.textFile("test/output.txt2")
