...
object Spark11_Oper10 {
  def main(args: Array[String]): Unit = {
   ...
   // println("缩减分区前 = " +listRDD.partitions.size)
    ...
    var coalesceRDD: RDD[Int] = listRDD.coalesce(3,true)
    val coalesceRDD1: RDD[Int]  = listRDD.repartition(4)

    println("缩减分区后 = " +coalesceRDD1.partitions.size)
    coalesceRDD1.collect().foreach(println)
